...
// Cut 10 — cut 9, but the demo no longer opens on the empty #welcome channel (it
// just buried the features). The Meet-kordeon morph and the establishing shot now
// materialise straight into the hero feature channel at the same early state the
// chat beat opens on, so the whole product act is one continuous scene on the
// feature. Everything else is reused unchanged from cut 9.
...
export const FILM_V10_DURATION = BEATS_TOTAL - (ORDER.length - 1) * CROSSFADE_FRAMES;

// Same copy fields as cut 9 (generic-agent PR review + pivot).
export const filmV10Schema = filmSchema.extend({
...
export const filmV10DefaultProps: z.infer<typeof filmV10Schema> = {
  ...filmDefaultProps,
...
export function LaunchFilmV10(props: z.infer<typeof filmV10Schema>) {
  return (
